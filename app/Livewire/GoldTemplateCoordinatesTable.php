...
class GoldTemplateCoordinatesTable extends Component implements HasActions, HasSchemas, HasTable
{
    ...
    public int $canvasWidth = 941;

    public int $canvasHeight = 1796;

    /**
     * @param  array<string, array{coordinate_key?: string, label: string, field: string, x: int, y: int}>  $rows
     */
    public function mount(array $rows = [], int $canvasWidth = 941, int $canvasHeight = 1796): void
    {
    ...
    protected function updateCoordinate(array $record, string $axis, mixed $state): int
    {
        ...
        $this->flushCachedTableRecords();
        $this->dispatch('gold-coordinate-input-updated', rows: $this->normalizeRows($this->rows));
    ...
    {
        return view('livewire.gold-template-coordinates-table');
    }
}
